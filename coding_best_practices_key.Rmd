...
2. Change `y` assignment symbol to <-
3. `additionResult` should be changed to `addition_result`
...
1. Change `average_x` assignment symbol to <-
2. Change `average_y` assignment symbol to <-
3. Fix parentheses for `mean( x, na.rm = TRUE )` to `mean(x, na.rm = TRUE)`
4. Add spacing between `na.rm=TRUE` argument in `mean( x, na.rm=TRUE )`
5. Fix parentheses for `mean(y )` to `mean(y)`
6. Explicity call `na.rm = TRUE` in `mean(y )`
...
1. Add space before bracket in `if` statement
2. Add space before bracket in `else` statement
3. Use " instead of ' for quoting text
4. Conditional statement is based on implicit type coercion
...
Step 1: In order to start using GitHub, you must first create a repository. *Create an empty public repository.*
